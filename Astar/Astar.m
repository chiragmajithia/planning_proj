...
% x_val = 1;
% y_val = 1;
function [Optimal_path] = Astar(Start,Goal,MAP)
[MAX_X, MAX_Y] = size(MAP);
% n=0;%Number of Obstacles
%

MAP(Goal(1),Goal(2))=0;%Initialize MAP with location of the target
...
for i=1:MAX_X
    for j=1:MAX_Y
        if(MAP(i,j) == 100)%Obstacle map value
            CLOSED(k,1)=i;
            CLOSED(k,2)=j;
            k=k+1;
        end
